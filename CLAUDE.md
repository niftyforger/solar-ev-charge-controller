...
- Resistor divider and comparator reference values (`R1`/`R2`, `R3`–`R5`, REF_CONN ≈2.5V, REF_EDGE ≈0.45V) for connector-state discrimination — chosen on paper against nominal 12V/9V/6V/3V/0V CP levels, not yet verified with a scope on the real Jueclat. `read_connector_state()`'s A/B/C/FAULT bucketing (`cp_interceptor.cpp`) depends on these landing where expected.
- `CP_CONNECTED_SENSE_GPIO`'s (and `CP_PWM_SENSE_GPIO`'s) idle-level polarity through the comparator/opto stage — `read_connector_state()` currently assumes HIGH = "above threshold" for both; confirm on the bench and flip the level checks if the opto stage inverts.
